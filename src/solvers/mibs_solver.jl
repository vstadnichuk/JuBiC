...
function process_mibs_log_text!(stats, logtext::String)
    match_nodes = match(r"Number of nodes fully processed:\s+(\d+)", logtext)
    if isnothing(match_nodes)
        match_nodes = match(r"Number of nodes processed:\s+(\d+)", logtext)
    end
    match_nodes_partial = match(r"Number of nodes partially processed:\s+(\d+)", logtext)
    match_nodes_branched = match(r"Number of nodes branched:\s+(\d+)", logtext)
    match_nodes_left = match(r"Number of nodes left:\s+(\d+)", logtext)
    match_tree_depth = match(r"Tree depth:\s+(\d+)", logtext)
    match_cpu_runtime = match(r"Search CPU time:\s+([0-9.]+)\s+seconds", logtext)
    match_runtime = match(r"Search wall-clock time:\s+([0-9.]+)", logtext)
    match_gap = match(r"Relative optimality gap is\s+([0-9.]+)%", logtext)
    match_vf = match(r"Number of problems \(VF\) solved\s*=\s*(\d+)", logtext)
    match_ub = match(r"Number of problems \(UB\) solved\s*=\s*(\d+)", logtext)
    match_hic = match(r"Number of Hypercube Intersection Cuts Generated:\s*(\d+)", logtext)
    match_gng = match(r"Number of Generalized No Good Cuts Generated:\s*(\d+)", logtext)
    match_bbc = match(r"Number of Benders Binary Cuts Generated:\s*(\d+)", logtext)

    nodes = isnothing(match_nodes) ? missing : parse(Int, match_nodes.captures[1])
    nodes_partial = isnothing(match_nodes_partial) ? missing : parse(Int, match_nodes_partial.captures[1])
    nodes_branched = isnothing(match_nodes_branched) ? missing : parse(Int, match_nodes_branched.captures[1])
    nodes_left = isnothing(match_nodes_left) ? missing : parse(Int, match_nodes_left.captures[1])
    tree_depth = isnothing(match_tree_depth) ? missing : parse(Int, match_tree_depth.captures[1])
    cpu_runtime = isnothing(match_cpu_runtime) ? missing : parse(Float64, match_cpu_runtime.captures[1])
    ...
    vf_solved = isnothing(match_vf) ? missing : parse(Int, match_vf.captures[1])
    ub_solved = isnothing(match_ub) ? missing : parse(Int, match_ub.captures[1])
    hic_cuts = isnothing(match_hic) ? missing : parse(Int, match_hic.captures[1])
    gng_cuts = isnothing(match_gng) ? missing : parse(Int, match_gng.captures[1])
    bbc_cuts = isnothing(match_bbc) ? missing : parse(Int, match_bbc.captures[1])

    _set_stat!(stats, "BNodes", nodes)
    _set_stat!(stats, "MibSNodesPartiallyProcessed", nodes_partial)
    _set_stat!(stats, "MibSNodesBranched", nodes_branched)
    _set_stat!(stats, "MibSNodesLeft", nodes_left)
    _set_stat!(stats, "MibSTreeDepth", tree_depth)
    _set_stat!(stats, "MibSCPURuntime", cpu_runtime)
    _set_stat!(stats, "runtime", runtime)
    _set_stat!(stats, "gap", gap)
    _set_stat!(stats, "MibSVFProblemsSolved", vf_solved)
    _set_stat!(stats, "MibSUBProblemsSolved", ub_solved)
    _set_stat!(stats, "MibSHypercubeIntersectionCuts", hic_cuts)
    _set_stat!(stats, "MibSGeneralizedNoGoodCuts", gng_cuts)
    _set_stat!(stats, "MibSBendersBinaryCuts", bbc_cuts)
end
