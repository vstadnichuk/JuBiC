...
function build_opt_cut(subLP::ConnectorLP, optL2, y_vals, x_vals, params::GBCparam, timelimit)
    ...
    if blc_subroutine
        ...
        cut -= ceil(Int, optL2 * gval)  # TODO: rounding should be save and avoid numeric trouble
        ...
        for a in keys(cutcoeff_BlC) 
            ...
            if params.trim_coeff
                cut -= ceil(min(coefa * gval, bigMterm)) * (1-master_vars[a]) * ceil(Int, y_vals[a]) # TODO: rounding should be save and avoid numeric trouble
            else
                cut -= ceil(Int, coefa * gval) * (1-master_vars[a]) * ceil(Int, y_vals[a]) # TODO: rounding should be save and avoid numeric trouble
            end
        end
        ...
        theta_xXg =
            ceil(Int, optL2 * gval) + sum(bigMterm * (1 - master_vars[a]) * y_vals[a] for a in subLP.A)  # The big M already contains the gval coefficient  # TODO: rounding should be save and avoid numeric trouble
        cut -= theta_xXg
    end
    ...
    if params.trim_coeff
        cut -= sum(ceil(Int, min(kvals[a], bigMterm)) * master_vars[a] for a in subLP.A) # TODO: rounding should be save and avoid numeric trouble
    else
        cut -= sum(ceil(kvals[a]) * master_vars[a] for a in subLP.A) # TODO: rounding should be save and avoid numeric trouble
    end
